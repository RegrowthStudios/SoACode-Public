...
FrameBuffer::FrameBuffer(i32 internalFormat, GLenum type, ui32 width, ui32 height, i32 msaa) :
    _vbo(0),
    _ibo(0),
    _width(width),
    _height(height),
...
void FrameBuffer::draw(const ui32v2& destDimensions, i32 drawMode /* = 0 */)
{
    if (_vbo == 0){
        glGenBuffers(1, &_vbo);
