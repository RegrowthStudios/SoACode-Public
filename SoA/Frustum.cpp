...
double worldFrustum[6][4];
double gridFrustum[6][4];

// ********************************* Frustum Functions ************************************
void ExtractFrustum(glm::dmat4 &projectionMatrix, glm::dmat4 &viewMatrix, double frustum[6][4])
{
    glm::dmat4 model = glm::dmat4(1.0) * projectionMatrix * viewMatrix;
...
{
   for (int p = 0; p < 4; p++ ) if(frustum[p][0] * x + frustum[p][1] * y + frustum[p][2] * z + frustum[p][3] <= 0 ) return false;
