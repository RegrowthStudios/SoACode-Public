    ...
    while (trunkStep <= treeHeight){
        ...
        for (int i = 0; i < coreWidth; i++){ //z
            ...
            for (int j = 0; j < coreWidth; j++){ //x
                if (tt->leafCapShape && trunkStep == treeHeight - leafRadius - 1){
                ...
                if ((tt->leafCapShape != 2 || trunkStep < treeHeight - leafRadius && trunkStep < treeHeight) && !(tt->leafCapShape == 4 && (!tt->isMushroomCapInverted && trunkStep >= treeHeight - 1)) && !(tt->leafCapShape == 3 && trunkStep >= treeHeight - 1)){
                    makeWood = 1;
                        ...
                        if (trunkStep == 0){ //roots
                            ...
                            if (r <= branchChance){
                                ...
                                if (trunkStep < branchStart + (treeHeight - branchStart) / 2){
                                    bdir = tt->bottomBranchDir;
                                } else{
                                    bdir = tt->topBranchDir;
                                }
                                if (bdir == 3){
                ...
                if (tt->leafCapShape == 2 && trunkStep > treeHeight - leafRadius * 2 - 1) //round leaves
                {
                    ...
                    if (code) return code;
                } else if (tt->leafCapShape == 1 && trunkStep == treeHeight){ //cluster leaves
                    code = makeCluster(ch2, leafRadius, c2, leafBlock);
                    if (code) return code;
                } else if (tt->leafCapShape == 3){ //pine leaves
                ...
                } else if (tt->leafCapShape == 4 && trunkStep == treeHeight){ //mushroom shape cap
...
void FloraGenerator::placeTreeNodes()
{
    ...
    for (Uint32 j = 0; j < wnodes.size(); j++){ //wood nodes
        ...
        if (c >= CHUNK_LAYER){
            if (owner->getBlockID(c - CHUNK_LAYER) == DIRTGRASS) owner->setBlockData(c - CHUNK_LAYER,  DIRT); //replace grass with dirt
        } else if (owner->bottom && owner->bottom->isAccessible){
            if (owner->bottom->getBlockID(c + CHUNK_SIZE - CHUNK_LAYER) == DIRTGRASS) owner->bottom->setBlockData(c + CHUNK_SIZE - CHUNK_LAYER, DIRT);
        }
    }
...
int FloraGenerator::makeTreeData(Chunk *chunk, TreeData &td, TreeType *tt) {
    ...
    td.botBranchChance = mod*(tt->bottomBranchChance.max - tt->bottomBranchChance.min) + tt->bottomBranchChance.min;
    td.topBranchChance = mod*(tt->topBranchChance.max - tt->topBranchChance.min) + tt->topBranchChance.min;
    td.botBranchLength = mod*(tt->bottomBranchLength.max - tt->bottomBranchLength.min) + tt->bottomBranchLength.min;
    td.topBranchLength = mod*(tt->topBranchLength.max - tt->topBranchLength.min) + tt->topBranchLength.min;
    td.botBranchWidth = mod*(tt->bottomBranchWidth.max - tt->bottomBranchWidth.min) + tt->bottomBranchWidth.min;
    td.topBranchWidth = mod*(tt->topBranchWidth.max - tt->topBranchWidth.min) + tt->topBranchWidth.min;
    td.treeType = tt;
...
{
    int code;
    if (makeNode && chunk->getBlockID(c) == NONE){
        lnodes.push_back(TreeNode(c, ntype, chunk));
    }
...
int FloraGenerator::makeBranch(Chunk *chunk, int step, int initStep, int c, int dir, bool makeNode, TreeData *td, int initDir, int initSize, int leafSizeMod, bool isRoot, int initBranchDir)
{
    ...
    while (1){
        ...
        if (step == 1 && !isRoot) //leaves
        {
            ...
            if (td->treeType->branchLeafShape == 2){
                code = makeSphere(ch2, c2, leafBlock, size + 3 + leafSizeMod);
            } else if (td->treeType->branchLeafShape == 1){
